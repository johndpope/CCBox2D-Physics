...
-(void) createCartesianBounds {
    ...
    // for the screenBorder body we'll need these values
    CGSize screenSize = [CCDirector sharedDirector].winSize;
    float widthInMeters = screenSize.width / PTM_RATIO*kBoundsRatioToScreen;
    float heightInMeters = screenSize.height / PTM_RATIO*kBoundsRatioToScreen;
   ...
   // NSLog(@"drawRect");
    ...
    if ( appDelegate.system ) {
        ...
        if (self.isDebugDrawing) {
           ...
           // [self createLineByRect:[self scaleRect:appDelegate.system.tweenBoundsTarget] color:ccc4(0, 255, 255, 255) tag:10];
            ...
            // view)
            //[self createLineByRect:[self scaleRect:appDelegate.system.tweenBoundsCurrent] color:ccc4(255, 255, 0, 255) tag:20];
...
-(void)createLineByRect:(CGRect)rect{
    [self createLineByRect:rect color:ccc4(0,255,0,255) tag:0];
}
-(void)createLineByRect:(CGRect)rect color:(ccColor4B)color tag:(NSInteger)tag
{
    ...
    if ((tag == 10) ||(tag == 20)){
        ...
        [self removeChildByTag:tag+1];
        [self removeChildByTag:tag+2];
        [self removeChildByTag:tag+3];
    }
